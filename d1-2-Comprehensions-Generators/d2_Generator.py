...
# Tình huống: Bạn có một file server log nặng 10GB. Bạn không thể dùng open().readlines() vì nó sẽ load cả 10GB vào RAM (máy sẽ treo). Giả lập: Chúng ta có một list log giả lập (coi như là các dòng trong file). Yêu cầu: Viết một generator function nhận vào danh sách log, chỉ yield ra những dòng có chứa chữ "ERROR".
